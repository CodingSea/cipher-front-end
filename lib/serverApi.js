...
async function updateChannel(serverId, id, data)
{
    try
    {
        const token = localStorage.getItem("token")
        const response = await axios.put(`${ import.meta.env.VITE_BACKEND_URL }/server/${ serverId }/channel/${ id }`, data, { headers: { Authorization: `Bearer ${ token }` } });
        return response.data;
    }
    catch (error)
    {
...
async function deleteChannel(serverId, id)
{
    try
    {
        const token = localStorage.getItem("token")
        const response = await axios.delete(`${ import.meta.env.VITE_BACKEND_URL }/server/${ serverId }/channel/${ id }`, { headers: { Authorization: `Bearer ${ token }` } });
        return response.data;
    }
    catch (error)
    {
