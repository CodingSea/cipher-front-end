...
async function createServer(data)
{
    try
    {
        const response = await axios.post(`${ import.meta.env.VITE_BACKEND_URL }/server/new`, data, { headers: { Authorization: `Bearer ${ token }` } });
        return response;
    }
    catch (error)
    {
...
async function getAllServers(userId)
{
    ...
    const url = `${ import.meta.env.VITE_BACKEND_URL }/server/:userId`;
